...
try:
    ...
    from src.fpe_del.del_model import AWP, FeedbackModelKD, FeedbackModelKDMsd
...
def run_training():
    ...
    # Student
    if config["use_msd"]:
        student = FeedbackModelKDMsd(config)
    else:
        student = FeedbackModelKD(config)
    ...
    if config["use_msd"]:
        teacher = FeedbackModelKDMsd(teacher_config)
    else:
        teacher = FeedbackModelKD(teacher_config)

    print("loading teacher from previously trained checkpoint...")
    checkpoint = config["teacher_ckpt"]
    print(f"teacher checkpoint path: {checkpoint}")
    ckpt = torch.load(checkpoint)
    ...
    student, teacher, optimizer, train_dl, valid_dl = accelerator.prepare(
        student, teacher, optimizer, train_dl, valid_dl
    )
    ...
    for epoch in range(num_epochs):
        ...
        for step, batch in enumerate(train_dl):
            logits, loss, loss_dict = student(**batch, teacher=teacher)
