...
try:
    from src.fpe_delv3_prompt.delv3_dataloader_prompt import (
        CustomDataCollatorWithPadding, CustomDataCollatorWithPaddingMaskAug)
    from src.fpe_delv3_prompt.delv3_dataset_prompt import get_fast_dataset
    from src.fpe_delv3_prompt.delv3_model_prompt import AWP, FeedbackModel
...
def run_training():
    ...
    # add prompt idx
    prompt_df = pd.read_csv(config["prompt_path"])
    prompt_df = prompt_df.drop_duplicates(subset=["essay_id"])
    ...
    if config["use_augmented_data"]:
        ...
        print(f"using {config['augmented_samples']} augmented samples...")
    ...
    # print add prompts to the essay
    prompt_map = dict(zip(prompt_df["essay_id"], prompt_df["prompt"]))
    essay_df["original_essay_id"] = essay_df["essay_id"].apply(lambda x: str(x).split("_")[0])
    essay_df["prompt"] = essay_df["original_essay_id"].map(prompt_map)
    remove_essay_ids = essay_df[essay_df["prompt"].isna()].essay_id.unique().tolist()
    print(f"# of essays with missing prompt = {len(remove_essay_ids)}")

    print(f"num essays before = {len(essay_df)}")

    essay_df = essay_df[~essay_df["essay_id"].isin(remove_essay_ids)].copy()
    essay_df = essay_df.drop(columns=["original_essay_id"])
    essay_df = essay_df.reset_index(drop=True)

    df = df[~df["essay_id"].isin(remove_essay_ids)].copy()
    df = df.reset_index(drop=True)

    print(f"num essays after = {len(essay_df)}")
    ...
    if args.use_wandb:
        ...
        wandb.save(f"{config['code_dir']}/src/fpe_delv3_prompt/*")
        wandb.save(f"{config['code_dir']}/configs/delv3_prompt/*")
