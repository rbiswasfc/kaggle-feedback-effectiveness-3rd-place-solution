...
try:
    from src.fpe_del_prompt_all.delv3_dataloader import (
        CustomDataCollatorWithPadding, CustomDataCollatorWithPaddingMaskAug)
    from src.fpe_del_prompt_all.delv3_dataset import get_fast_dataset
    from src.fpe_del_prompt_all.delv3_model import AWP, FeedbackModel
    from src.fpe_del_prompt.del_model import FeedbackModel as FeedbackModelDEL
...
def run_training():
    ...
    if 'v3' in config['base_model_path']:
        model = FeedbackModel(config)
    else:
        model = FeedbackModelDEL(config)
