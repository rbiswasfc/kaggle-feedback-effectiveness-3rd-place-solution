...
try:
    from src.fpe_fast_prompt.fast_dataloader import (
        CustomDataCollatorWithPadding, CustomDataCollatorWithPaddingMaskAugCutmix, CustomDataCollatorWithPaddingMaskAug)
    from src.fpe_fast_prompt.fast_dataset import get_fast_dataset
    from src.fpe_fast_prompt.fast_model import (AWP, FeedbackModel,
                                         FeedbackModelStudentFT, FeedbackModelSlidingWindow)
...
topic_map_new = {
	'seagoing luke animals cowboys': 'Do you think joining the Seagoing cowboys program is a good option?',
	'driving phone phones cell' :  'Do you think drivers can be allowed to use their cell phones while driving?',
	 'phones cell cell phones school': 'Do you think students should (Policy 1) or should not (Policy 2) be allowed cell phone use in class?',
	 'straights state welfare wa' : ' State welfare' ,
	 'summer students project projects': 'Should summer projects be designed by students or teachers?',
	 'students online school classes': 'Which method of learning do you think is better for students? Online education (distance learning) or education in traditional classrooms?',
	 'car cars usage pollution': 'Do you think we should we limit car usage or not?',
	 'cars driverless car driverless cars': 'Do you think the development of driverless cars should be permitted?',
	 'emotions technology facial computer' : 'Do you think we need a Facial Action Coding System in our classrooms?',
	 'community service community service help': 'Do you think that community service should be mandatory for high school students or not?',
	 'sports average school students' : 'Do you think only students who maintain at least B grades should be allowed to play sports? Is this policy change good?',
	 'advice people ask multiple': 'When people ask for advice, they sometimes talk to more than one person. Why does seeking multiple opinions can help someone make a better choice?',
	 'extracurricular activities activity students': 'Should all students participate in at least one extra-curricular after school activity?',
	 'electoral college electoral college vote':  'Are you in favour of keeping the Electoral College or changing to election by popular vote for the president of the United States?' ,
	 'electoral vote college electoral college' : 'Are you in favour of keeping the Electoral College or changing to election by popular vote for the president of the United States?' ,
	 'face mars landform aliens' : 'Is the "Face on Mars" a natural landform?',
     'venus planet author earth': 'Do you think exploring Venus is a worthy pursuit according to the author?',
}
...
def run_training():
    ...
    elif config["use_cutmix_aug"]:
        data_collector_train = CustomDataCollatorWithPaddingMaskAugCutmix(tokenizer=tokenizer)
    else:
    ...
    if config.get("use_student", False):
        print("=="*40)
        model = FeedbackModelStudentFT(config)
        print("using FeedbackModelStudentFT...")
        ckpt = torch.load(config["student_path"])
        print(f"loss in checkpoint: {ckpt['lb']}")
        ...
        if config['use_sliding_window']:
            model = FeedbackModelSlidingWindow(config)
        else:
