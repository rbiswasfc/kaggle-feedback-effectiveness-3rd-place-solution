...
def tokenizer_test(tokenizer):
    ...
    print(
        f"tokenizer test: {tokenizer.tokenize('Starts: [TOPIC] [SOE] [LEAD] [CLAIM]')}")
    print(
        f"tokenizer test: {tokenizer.tokenize('Starts: [EOE] [LEAD_END] [==END==]')}")

    print("=="*40)


def insert_tokens(essay_id, essay_text, prompt, anno_df):
    tok_map = {
        "Lead": ["[LEAD]", "[LEAD_END]"],
        "Position": ["[POSITION]", "[POSITION_END]"],
        "Claim": ["[CLAIM]", "[CLAIM_END]"],
        "Counterclaim": ["[COUNTER_CLAIM]", "[COUNTER_CLAIM_END]"],
        "Rebuttal": ["[REBUTTAL]", "[REBUTTAL_END]"],
        "Evidence": ["[EVIDENCE]", "[EVIDENCE_END]"],
        "Concluding Statement": ["[CONCLUDING_STATEMENT]", "[CONCLUDING_STATEMENT_END]"]
    }
    ...
    essay_text = f"[TOPIC] {prompt} {essay_text}"
...
def execute_mlm(config):
    ...
    print(f"# of essays before removal: {len(notes_df)}")
    tmp_df = pd.merge(anno_df, notes_df, on="essay_id", how="left")
    remove_essay_ids = tmp_df[tmp_df["essay_text"].isna()].essay_id.unique().tolist()
    tmp_df = tmp_df[tmp_df["essay_id"].isin(remove_essay_ids)].copy()
    remove_essay_ids.extend(tmp_df[tmp_df["essay_text"].apply(len) <= 5].essay_id.unique().tolist())
    print(f"warning!! the following essay ids will be removed: {remove_essay_ids}")
    ...
    print(f"# of essays after removal: {len(notes_df)}")
    ...
    if config["load_dataset_from_cache"]:
        ...
        notes_df.sample(128).to_csv("./notes_sample_delv3.csv", index=False)
        print(notes_df.head())
        ...
        mlm_dataset = get_mlm_dataset(notes_df, config, tokenizer)
    ...
    if config["load_from_ckpt"]:
        print(f"loading from previously trained checkpoint: {config['model_checkpoint']}")
        ...
        model.cls = DebertaV2OnlyMLMHead(base_config)  # TODO: check impact
