...
def process_input_df(anno_df, notes_df, topics_map):
    ...
    anno_df = pd.merge(anno_df, notes_df, on="essay_id", how="left")
    anno_df["topic_num"] = anno_df["essay_id"].map(topics_map)

    keep_cols = ["uid", "discourse_id", "discourse_type", "topic_num"]
    if "discourse_effectiveness" in anno_df.columns:
        keep_cols.append("discourse_effectiveness")

    grouped_df = anno_df.groupby("essay_id")[keep_cols].agg(list).reset_index()

    grouped_df = pd.merge(grouped_df, notes_df, on="essay_id", how="left")
    grouped_df = grouped_df.rename(columns={"uid": "uids", "discourse_id": "discourse_ids", "topic_num": "topic_nums"})
...
class FeedbackDatasetMeta:
    ...
    def get_dataset(self, df, essay_df, feature_map, topics_map, mode='train'):
        ...
        df = process_input_df(df, essay_df, topics_map)
