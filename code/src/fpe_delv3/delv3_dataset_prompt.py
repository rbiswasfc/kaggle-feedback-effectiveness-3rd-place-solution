...
def process_essay(essay_id, essay_text, prompt_idx, anno_df):
    ...
    essay_text = "[SOE] " + f"[PROMPT={prompt_idx}] " + essay_text + " [EOE]"
    return essay_text


def process_input_df(anno_df, notes_df, prompt_df):
    ...
    notes_df = pd.merge(notes_df, prompt_df, on="essay_id", how="left")
    notes_df["essay_text"] = notes_df[["essay_id", "essay_text", "prompt_idx"]].apply(
...
class AuxFeedbackDataset:
    ...
    def get_dataset(self, df, essay_df, prompt_df, mode='train'):
        ...
        :rtype: Dataset
        """
        df = process_input_df(df, essay_df, prompt_df)
...
def get_fast_dataset(config, df, essay_df, prompt_df, mode="train"):
    """Function to get fast approach dataset with truncation & sliding window
    """
    dataset_creator = AuxFeedbackDataset(config)
    _, task_dataset = dataset_creator.get_dataset(df, essay_df, prompt_df, mode=mode)
