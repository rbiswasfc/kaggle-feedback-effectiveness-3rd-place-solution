...
from transformers.models.luke.modeling_luke import LukeConfig, LukeEncoder
...
class FeedbackModelLukeAttention(nn.Module):
    """
    ...
    def __init__(self, config):
        ...
        super(FeedbackModelLukeAttention, self).__init__()
        self.config = config
        ...
        self.fpe_span_attention = LukeEncoder(attention_config)  # LukeAttention(attention_config)
    ...
    def get_luke_attention_mask(self, word_attention_mask, entity_attention_mask):
        ...
        extended_attention_mask = (1.0 - extended_attention_mask) * torch.finfo(torch.float32).min  # changed
    ...
    def forward(
        ...
        **kwargs
        ...
        # get luke style attention mask
        luke_attention_mask = self.get_luke_attention_mask(attention_mask, span_attention_mask)

        luke_attention_outputs = self.fpe_span_attention(
            word_hidden_states=encoder_layer,
            entity_hidden_states=mean_feature_vector,
            attention_mask=luke_attention_mask,
        )
        # pdb.set_trace()
        luke_feature_vector = luke_attention_outputs[1]
        # feature_vector = torch.cat([feature_vector, luke_feature_vector], dim=-1)
        feature_vector = luke_feature_vector
...
class FeedbackModelTwoAttention(nn.Module):
    """
    The feedback prize effectiveness model for fast approach
    """
    ...
    def __init__(self, config):
        ...
        super(FeedbackModelTwoAttention, self).__init__()
        self.config = config
        ...
        self.fpe_span_attention_luke = LukeEncoder(attention_config)  # LukeAttention(attention_config)
        ...
        self.fpe_span_attention_bert = BertEncoder(attention_config)  # , position_embedding_type="relative_key")
        ...
        self.classifier = nn.Linear(feature_size*2, self.num_labels)  # luke + bert
    ...
    def forward(
        ...
        **kwargs
        ...
        luke_attention_outputs = self.fpe_span_attention_luke(
            word_hidden_states=encoder_layer,
            entity_hidden_states=mean_feature_vector,
            attention_mask=luke_attention_mask,
        )
        ...
        bert_feature_vector = self.fpe_span_attention_bert(
            mean_feature_vector, extended_span_attention_mask)[0]  # bert encoder

        feature_vector = torch.concat([luke_feature_vector, bert_feature_vector], dim=-1)
...
class FeedbackModelAddedEmbeddings(nn.Module):
    """
    The feedback prize effectiveness model for fast approach
    """
    ...
    def __init__(self, config):
        ...
        super(FeedbackModelAddedEmbeddings, self).__init__()
        self.config = config
        ...
        self.fpe_span_attention = BertEncoder(attention_config)  # , position_embedding_type="relative_key")
        ...
        # embeddings
        self.max_span_count = 64
        self.num_discourse_types = 8
        self.span_position_embeddings = nn.Embedding(self.max_span_count, self.base_model.config.hidden_size)
        self.span_discourse_embeddings = nn.Embedding(
            self.num_discourse_types, self.base_model.config.hidden_size, padding_idx=0)

        # initialize the embeddings
        self.span_position_embeddings.weight.data.normal_(mean=0.0, std=self.base_model.config.initializer_range)
        self.span_discourse_embeddings.weight.data.normal_(mean=0.0, std=self.base_model.config.initializer_range)
    ...
    def forward(
        ...
        **kwargs
        ...
        # add embeddings here
        # pdb.set_trace()
        span_position_ids = create_position_ids_span_mask(span_attention_mask)
        span_position_embeddings = self.span_position_embeddings(span_position_ids)
        span_discourse_embeddings = self.span_discourse_embeddings(discourse_type_ids)

        mean_feature_vector = (mean_feature_vector + span_position_embeddings + span_discourse_embeddings)/3
        ...
        extended_span_attention_mask = span_attention_mask[:, None, None, :]
        extended_span_attention_mask = (1.0 - extended_span_attention_mask) * -10000.0
        feature_vector = self.fpe_span_attention(mean_feature_vector, extended_span_attention_mask)[0]  # bert encoder
...
#     def compute_loss(self, logits, labels, span_attention_mask):
#         # compute logits and loss
#         loss_dict = dict()
#         loss_dict["multitask_loss"] = None
#         loss_dict["ce_loss"] = None
