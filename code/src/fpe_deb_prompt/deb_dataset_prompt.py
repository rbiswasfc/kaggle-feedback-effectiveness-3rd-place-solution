...
def get_tokenizer(config):
    ...
    print(
        f"tokenizer test: {tokenizer.tokenize('Starts: [TOPIC] this is an essay [SOE] [LEAD] [CLAIM]')}")
    print(
...
TOKEN_MAP = {
    "Lead": ["[LEAD] Does the lead grab the readers attention and point towards the position?", "[LEAD_END]"],
    "Position": ["[POSITION] Does the position state a clear stance closely related to the prompt?", "[POSITION_END]"],
    "Claim": ["[CLAIM] Is the claim closely relevant to the position and backs up the position with specific points or perspectives? Is the claim valid and acceptable?", "[CLAIM_END]"],
    "Counterclaim": ["[COUNTER_CLAIM] Is the counterclaim reasonable and relevant to the position?", "[COUNTER_CLAIM_END]"],
    "Rebuttal": ["[REBUTTAL] Does the rebuttal directly answer and refute the counterclaim?", "[REBUTTAL_END]"],
    "Evidence": ["[EVIDENCE] Is the evidence closely related  to the claim and provides valid examples?", "[EVIDENCE_END]"],
    "Concluding Statement": ["[CONCLUDING_STATEMENT] Does the conclusion effectively restates the claims using different wording?", "[CONCLUDING_STATEMENT_END]"]
}
