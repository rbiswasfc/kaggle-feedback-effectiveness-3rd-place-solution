...
#-------- AWP ------------------------------------------------------------#
def reinit_del(base_model, num_reinit_layers):
...
def reinit_dexl(base_model, num_reinit_layers):
    config = base_model.config
...
class FeedbackModelKDMsd(nn.Module):
    """
    ...
    def __init__(self, config):
        ...
        super(FeedbackModelKDMsd, self).__init__()
        self.config = config
        ...
        if config["num_layers_reinit"] > 0:
            ...
            reinit_del(self.base_model, self.config["num_layers_reinit"])
            print("=="*40)
    ...
    def forward(
        ...
        **kwargs
        ...
        logits, features = self.get_logits(
            input_ids,
            token_type_ids,
            attention_mask,
            span_head_idxs,
        ...
        feature_vector1 = self.dropout1(features)
        feature_vector2 = self.dropout2(features)
        feature_vector3 = self.dropout3(features)
        feature_vector4 = self.dropout4(features)
        feature_vector5 = self.dropout5(features)
    ...
    def forward(
        ...
        **kwargs
        ...
        if labels is not None:
            ...
            kld_inputs = F.log_softmax(logits/self.config["temperature"], dim=-1)
            kld_targets = F.softmax(t_logits/self.config["temperature"], dim=-1)
            kd_loss = torch.sum(self.kd_loss(kld_inputs, kld_targets), dim=-1)
            kd_loss = torch.sum(kd_loss*span_attention_mask.float())
            kd_loss = kd_loss / torch.sum(span_attention_mask.float()).clamp(min=1.0)
...
class ResidualLSTM(nn.Module):
    """
    source: https://github.com/Shujun-He/3rd_Solution_Feedback_Prize_Evaluating_Student_Writing
    """

    def __init__(self, d_model, dropout_rate=0.1):
        super(ResidualLSTM, self).__init__()
        self.downsample = nn.Linear(d_model, d_model//2)
        self.LSTM = nn.GRU(d_model//2, d_model//2, num_layers=2, bidirectional=False, dropout=dropout_rate)
...
class FeedbackModel(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config["num_layers_reinit"] > 0:
            print(f"re-initializing last {self.config['num_layers_reinit'] } layers of the base model...")
            reinit_dexl(self.base_model, self.config["num_layers_reinit"])
            print("=="*40)
        ...
        # # LSTM Head
        if self.config["use_residual_lstm"]:
            print("=="*40)
            print("using residual lstm block...")
            self.fpe_lstm_layer = ResidualLSTM(hidden_size)
            print("=="*40)
        else:
            self.fpe_lstm_layer = nn.LSTM(
                input_size=feature_size,
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.config["use_residual_lstm"]:
            self.fpe_lstm_layer.LSTM.flatten_parameters()
            encoder_layer = self.fpe_lstm_layer(encoder_layer)  # LSTM layer outputs
        ...
        mean_feature_vector = []
        #print(f'LSTM shape: {encoder_layer.shape}  head_idx: {span_head_idxs} tail idx: {span_tail_idxs}')
        #print(f'input_ids: {input_ids}')
        for i in range(bs):
            span_vec_i = []
            for head, tail in zip(span_head_idxs[i], span_tail_idxs[i]):
                #print(f'i: {i} head_idx: {head} tail idx: {tail}')
                # span feature
                tmp = torch.mean(encoder_layer[i, head+1:tail], dim=0)  # [h]
                span_vec_i.append(tmp)
                #print(f'Span vec shape: {len(span_vec_i)}')
            span_vec_i = torch.stack(span_vec_i)  # (num_disourse, h)
            mean_feature_vector.append(span_vec_i)

        mean_feature_vector = torch.stack(mean_feature_vector)  # (bs, num_disourse, h)
        #print(f'Mean ft vec shape: {mean_feature_vector.shape}')
        mean_feature_vector = self.layer_norm(mean_feature_vector)
        ...
        if self.config['MSD']:
            ...
            #print(f'Logits shape: {logits.shape}') # bs x num discourse x num labels
