...
def process_input_df(anno_df, notes_df):
    ...
    anno_df = anno_df[["discourse_id", "essay_id", "discourse_text", "discourse_start", "discourse_end",
                       "discourse_type", "discourse_effectiveness", "uid", "confidence", "sd"]].copy()
    ...
    # all_discourse_ids = list(chain(*anno_df["discourse_id"].values))
    # all_discourse_spans = list(chain(*anno_df["span"].values))
    ...
    anno_df.sample(frac=0.001).to_csv("./sample_df.csv", index=False)

    grouped_df = anno_df.groupby("essay_id")[["uid", "discourse_effectiveness",
                                              "discourse_type", "confidence", "sd"]].agg(list).reset_index()
...
class AuxFeedbackDatasetPseudo:
    ...
    def generate_confidence_sd_scores(self, examples):
        confidence_scores, sd_scores = [], []
        for example_confidence_scores, example_sd_scores in zip(examples["confidence"], examples["sd"]):
            confidence_scores.append(example_confidence_scores)
            sd_scores.append(example_sd_scores)
        return {"confidence_scores": confidence_scores, "sd_scores": sd_scores}
    ...
    def get_dataset(self, df, essay_df, mode='train'):
        ...
        task_dataset = task_dataset.map(self.generate_confidence_sd_scores, batched=True)
...
def get_fast_dataset_pseudo(config, df, essay_df, mode="train"):
    """Function to get fast approach dataset with truncation & sliding window
    """
    dataset_creator = AuxFeedbackDatasetPseudo(config)
    ...
    def recompute_confidence_sd_scores(examples):
        confidence_scores, sd_scores = [], []
        ...
        for example_span_head_char_start_idxs, example_overflow_to_sample_mapping in zip(
            ...
            original_example = original_dataset[example_overflow_to_sample_mapping]
            original_example_span_head_char_start_idxs = original_example["span_head_char_start_idxs"]
            original_example_confidence_scores = original_example["confidence_scores"]
            original_example_sd_scores = original_example["sd_scores"]

            char2confidence = {k: v for k, v in zip(
                original_example_span_head_char_start_idxs, original_example_confidence_scores)}

            char2sd = {k: v for k, v in zip(
                original_example_span_head_char_start_idxs, original_example_sd_scores)}

            current_example_confidence_scores = [char2confidence[char_idx]
                                                 for char_idx in example_span_head_char_start_idxs]

            current_example_sd_scores = [char2sd[char_idx]
                                         for char_idx in example_span_head_char_start_idxs]
            confidence_scores.append(current_example_confidence_scores)
            sd_scores.append(current_example_sd_scores)
    ...
    task_dataset = task_dataset.map(recompute_confidence_sd_scores, batched=True)
