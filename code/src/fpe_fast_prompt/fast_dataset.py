...
if USE_NEW_MAP:
    ...
    TOKEN_MAP_QUERIES = {
        ...
        "Position": ["Position: Does the position state a clear stance closely related to the topic? [POSITION]", "[POSITION END]"],
        "Claim": ["Claim: Is the claim closely relevant to the position and backs up the position with specific points or perspectives. Is the claim valid and acceptable? [CLAIM]", "[CLAIM END]"],
        "Counterclaim": ["Counterclaim: Is the counterclaim reasonable and relevant to the position? [COUNTER_CLAIM]", "[COUNTER_CLAIM END]"],
        "Rebuttal": ["Rebuttal: Does the rebuttal directly answer and refute the counterclaim? [REBUTTAL]", "[REBUTTAL END]"],
        "Evidence": ["Evidence: Is the evidence closely related  to the claim, counterclaim or rebuttal and provides valid examples? [EVIDENCE]", "[EVIDENCE END]"],
        "Concluding Statement": ["Concluding Statement: Does the concluding statement effectively restates the claim using different wording? [CONCLUDING_STATEMENT]", "[CONCLUDING_STATEMENT END]"]
    }
...
QUERY_MAP = {
    "Lead" :  "[QUERY] Does the Lead grab the readers attention and point towards the Position ? [QUERY END]",
	"Position" :  "[QUERY] Does the Position state a clear stance closely related to the Topic ?  [QUERY END]",
	"Claim" :  "[QUERY]  Is the Claim closely relevant to the Position and backs up the Position with specific points or perspectives. Is the Claim valid and acceptable ? [QUERY END]",
	"Counterclaim" :  "[QUERY]  Is the Counterclaim reasonable and relevant to the Position ? [QUERY END]",
	"Rebuttal"  : "[QUERY] Does the Rebuttal directly answer and refute the Counterclaim ? [QUERY END]",
	"Evidence" : "[QUERY]  Is the Evidence closely related  to the Claim, Counterclaim or Rebuttal and provides valid examples? [QUERY END]",
    "Concluding Statement" : "[QUERY]  Does the Concluding Statement effectively restates the Claim using different wording? [QUERY END]",
}
...
def process_essay(essay_id, essay_text, topic, anno_df, config):
    ...
    buffer = 0
    #print(f'Essay id: {essay_id} topic: {topic}')
...
class AuxFeedbackDataset:
    ...
    def load_tokenizer(self):
        ...
        """
        if self.config["use_queries"]:
            print("adding new tokens...")
            tokens_to_add = []
            for this_tok in ['[QUERY]', '[QUERY END]']:
                 tokens_to_add.append(AddedToken(this_tok, lstrip=True, rstrip=False))
            self.tokenizer.add_tokens(tokens_to_add)
        """
